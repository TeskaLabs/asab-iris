...
class JinjaFormatterService(asab.Service, FormatterABC):
    ...
    def __init__(self, app, service_name="JinjaService"):
        ...
        try:
            self.Variables = {option.upper(): asab.Config.get('variables', option) for option in asab.Config.options('variables')}
        ...
        if json_path and os.path.exists(json_path):
            with open(json_path, 'r') as json_file:
                self.Variables.update({k.upper(): v for k, v in json.load(json_file).items()})
    ...
    def __getitem__(self, key):
        ...
        """
        return self.Variables.get(key.upper(), None)
