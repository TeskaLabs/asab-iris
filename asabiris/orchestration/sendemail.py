...
class SendEmailOrchestrator:
    ...
    async def send_email(self, email_to: List[str], body_template: str, email_from=None, email_cc=None, email_bcc=None,
        ...
        except (Exception, jinja2.exceptions) as e:
            L.warning("Error during template rendering: {}".format(str(e)))
            body_html, email_subject_body = self._generate_error_message(e, email_to)
            attachments = []  # Clear attachments if there's an error
    ...
    def _generate_error_message(self, e, email_to):
        error_message = (
            "Hello!<br><br>"
