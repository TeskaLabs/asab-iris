...
def handle_template_error(func):
	async def wrapper(self, *args, **kwargs):
		try:
			...
			return self._generate_error_message(e, args[0])
...
class SendEmailOrchestrator:
	...
	async def _render_template(self, template: str, params: Dict) -> Tuple[str, str]:
		...
		return {
			...
			'.md': lambda x: (self.services['MarkdownToHTMLService'].format(utils.find_subject_in_md(x)[0]),
	...
	def _process_attachments(self, attachments: List[Dict]) -> List[Tuple[Union[str, bytes], str, str]]:
		...
		for a in attachments:
			if 'base64' in a:
				processed_attachments.append((base64.b64decode(a['base64']),
	...
	def _generate_error_message(self, e: Exception, email_to: List[str]) -> Tuple[str, str]:
		error_message = (
			...
			"<br>Best regards,<br>LogMan.io".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
													', '.join(email_to), str(e))
		)
